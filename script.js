...
// --- FLUJO: PANEL DE ADMINISTRACIÓN (DASHBOARD) ---
...
async function publicarProductoForm() {
    ...
    if(!datosProducto.nombre || !datosProducto.precio || !datosProducto.imagen) {
        return alert("Completa el nombre, precio y la URL de la imagen.");
    }
...
function abrirModalEdicion(productoId) {
    ...
    document.getElementById('edit-prod-img').value = prod.img;
...
async function actualizarProductoForm() {
    const datosModificados = {
        ...
        imagen: document.getElementById('edit-prod-img').value.trim()
    };

    if(!datosModificados.nombre || !datosModificados.precio || !datosModificados.imagen) {
        return alert("Por favor, llena los campos obligatorios del producto.");
    }
